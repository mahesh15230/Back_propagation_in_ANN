...
def forwardpass(normal_minibatch, no_act_layers, layers, weights, actfunc, minibatchsize, reg_coeff, plot = False):
	...
	error = np.subtract(dkn, actfunc[i+1](layers[i+1], False))
	m = 0
...
print('dim fulldata', np.shape(fulldata))
ann_arch = [5,4,3,2,1]
...
def trainingANN(data, epochs, minibatchsize, beta, learning_param, reg_coeff, ann_arch, no_act_layers, layers, actfunc, weights):
	for j in range(epochs):
		...
		for i in range(len(traindata)):
			...
			trainingerror.append(training_error)
			backprop(error, beta, no_act_layers, act, weights, layers, learning_param, minibatchsize, reg_coeff)
		for i in range(len(validdata)):
			validation_error = forwardpass(validdata[:,i:i + minibatchsize], no_act_layers, layers, weights, actfunc, minibatchsize, reg_coeff)
			validerror.append(validation_error)
